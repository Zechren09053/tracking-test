...
.version-backup-container { display: flex; justify-content: space-between; align-items: center; margin-top: 30px; }
.version-info { color: #aaa; font-size: 0.8em; }
...
/* Modal Styles */
.modal { display: none; position: fixed; z-index: 2000; width: 100%;background-color: rgba(0,0,0,0.7); }
.modal-content { position: relative; background-color: #333; margin: 5% auto; padding: 0; width: 90%; max-width: 900px; border-radius: 8px; box-shadow: 0 4px 15px rgba(0,0,0,0.5); }
.modal-header { background-color: #444; padding: 20px; border-bottom: 1px solid #555; border-radius: 8px 8px 0 0; }
.modal-header h2 { margin: 0; color: #f5f5f5; }
.close { color: #aaa; float: right; font-size: 28px; font-weight: bold; cursor: pointer; }
.close:hover, .close:focus { color: #fff; }
.modal-body { padding: 20px; }
.modal-body iframe { width: 100%; height: 500px; border: none; background-color: #222; border-radius: 4px; }
.password-prompt { background-color: #444; padding: 20px; border-radius: 8px; margin-bottom: 20px; }
.password-prompt h3 { margin-top: 0; color: #f5f5f5; }
.password-prompt .form-group { margin-bottom: 15px; }
.password-error { color: #ff6b6b; font-size: 0.9em; margin-top: 5px; display: none; }
    </style>
</head>
<body>
        ...
        <div class="container">
            ...
            <div class="main" id="main-content">
                ...
                <div class="content">
                    ...
                    <div class="version-backup-container">
                        <div class="version-info">
                            <p>Application Version: <?= APP_VERSION ?></p>
                        </div>
                        <div class="backup-controls">
                            <button type="button" class="btn btn-secondary" id="backupRestoreBtn">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div id="passwordPrompt" class="password-prompt">
                    <h3>Enter Password to Access Backup & Restore</h3>
                    <div class="form-group">
                        <label for="modalPassword">Password:</label>
                        <input type="password" id="modalPassword" class="form-control" placeholder="Enter your password">
                ...
                <div id="backupContent" style="display: none;">
                    <iframe id="backupFrame" src="" frameborder="0"></iframe>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.close, #cancelBtn').click(function() {
                $('#bkupmodal').hide();
                $('#backupFrame').attr('src', ''); // Clear iframe source
            });

            // Close modal when clicking outside
            $(window).click(function(event) {
                if (event.target.id === 'bkupmodal') {
                    $('#bkupmodal').hide();
                    $('#backupFrame').attr('src', ''); // Clear iframe source
                }
            });
            ...
            $('#verifyPasswordBtn').click(function() {
                ...
                $.ajax({
                    url: 'verify_password.php', // You'll need to create this file
                    method: 'POST',
                    ...
                    success: function(response) {
                        if (response.success) {
                            $('#passwordPrompt').hide();
                            $('#backupContent').show();
                            $('#backupFrame').attr('src', 'backup_restore.php');
                        } else {
